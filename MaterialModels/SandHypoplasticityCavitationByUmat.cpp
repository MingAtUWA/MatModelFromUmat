...
int SandHypoplasticityCavitationByUmat::init(const char *dll_path)
{
	...
	if (!umat_dll)
		throw std::exception("SandHypoplasticityCavitationByUmat::"
			"can't load library\n");

    // resolve function address here
	dll_inte_func = (SandHypoIntegrateFunc)GetProcAddress(
		umat_dll, "SAND_HYPOPLASTICITY_CAVITATION_INTEGRATION");
    if (!dll_inte_func)
	{
		FreeLibrary(umat_dll);
		umat_dll = nullptr;
		throw std::exception("SandHypoplasticityCavitationByUmat::"
			"can't load function SAND_HYPOPLASTICITY_CAVITATION_INTEGRATION\n");
	}

	return 0;
}

void SandHypoplasticityCavitationByUmat::free()
{
...
SandHypoplasticityCavitationByUmat::SandHypoplasticityCavitationByUmat() :
	MaterialModel(&SandHypoplasticityCavitationByUmat_integration_func,
		"SandHypoplasticityCavitationByUmat")
{
...
void SandHypoplasticityCavitationByUmat::set_param(
...
int SandHypoplasticityCavitationByUmat_integration_func(MaterialModel* _self, double dstrain[6])
{
	SandHypoplasticityCavitationByUmat& self = *static_cast<SandHypoplasticityCavitationByUmat *>(_self);
